...
$(window).resize(function () {
    ...
    map.invalidateSize(); countymap.invalidateSize();
});
...
$(document).ready(function () {
    ...
    $('.legPanel').each(function(){
        ...
        if (loadall == loadchecked) {
            $(this).closest('.panel').find('.checked_all').html('<div class="chkicon glyphicon glyphicon-check"></div>');
            $(this).closest('.panel-body').append('<input type="hidden" class="Chkd" value="false" />');
        } else {
            $(this).closest('.panel').find('.checked_all').html('<div class="chkicon glyphicon glyphicon-unchecked"></div>');
            $(this).closest('.panel-body').append('<input type="hidden" class="Chkd" value="true" />');
        }
        var cbo = $(this).find('.dynacheck').length;
            if (cbo > 0){
                $(this).find('input.layer:checked').siblings('.dynacheck').find('.legend-check').html('<i class="glyphicon glyphicon-check"></i>');
            }
        });*/
        ...
        var $element = $(this);
            if (listPanel.attr('value') == 'true') {
                loadBar().done(function() {
                    $element.children().attr('class', 'chkicon glyphicon glyphicon-check');
                    $(listPanel).siblings('.checkbox').children('input').prop('checked', true).change();
                    $(listPanel).val('false');
                    $("#loading").fadeOut(150);
                });
            } else {
                $element.children().attr('class', 'chkicon glyphicon glyphicon-uchecked');
...
$('.layer').change(function () {
    ...
    if (all == checked) {
        $(this).closest('.panel').find('.checked_all').children().attr('class', 'chkicon glyphicon glyphicon-check');
        $(this).closest('.panel-body').find('.Chkd').val('false');
    } else {
        $(this).closest('.panel').find('.checked_all').children().attr('class', 'chkicon glyphicon glyphicon-unchecked');
        $(this).closest('.panel-body').find('.Chkd').val('true');
    }
    /*var cbo = $(this).siblings('.dynacheck').length;
    if (cbo > 0){
        if($(this).is(':checked')){
            $(this).siblings('.dynacheck').find('.legend-check i').attr('class', 'glyphicon glyphicon-check');
        }else{
            $(this).siblings('.dynacheck').find('.legend-check i').attr('class', 'glyphicon glyphicon-unchecked');
        }
    }*/
});
...
function toggleinfo(e) {
    ...
    var h = document.getElementById('info').offsetHeight + document.getElementById('infoheader').offsetHeight+5;
