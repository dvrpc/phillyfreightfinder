...
$(function() {
      ...
      d3.csv("data/d3/maritimeIndicators.csv", function (data) {
        ...
        $('#mi-year-select').html(maxDataYear+ ' <span class="caret"></span>');
        for(var i = 2003; i <= maxDataYear; i++ ){
          $('#maritime-year-dropdown').append('<li><input type="radio" id="m'+ i +'" name="county" value="'+ i +'" checked><label for="m'+ i +'">'+ i +'</label></li>'); 
        }
      ...
      function updateTradePanel(){
        ...
        if(data[yr].port_rank !== ""){
          ...
          var portRank = data[yr].port_rank,
            ...
            tradeComparison = calcDifference(nationalTradeChange, regionalTradeChange),
            ...
            exportChange = ((exportPercent - prevExport) / prevExport)*100,
            //export indicators
        ...
        if(tradeComparison < -2){
          $('#mi-total-icon').html(ind_down); 
        }else if(tradeComparison > 2){
        ...
        $('#mi-foreign-percent').html((foreignChange).toFixed(1) + '%');
         $('#mi-foriegn-indicator').html(calcChange(foreignChange));
        if(foreignChange > 3){
          $('#mi-foreign-icon').html(ind_up); 
        }else if(foreignChange < -3){
