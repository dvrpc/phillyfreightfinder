...
var hp_map, hp_metric = 'Speed', hp_time = '0', tg = 0, tileGroups=[]; 
...
var time_slider = document.getElementById('hp_timing_slider');
...
// add event listener for timing change
		...
		$(document.body).on('click', '.hp_play', function() {
			$(this).toggleClass('hp_play hp_pause').html('<i class="glyphicon glyphicon-pause"></i>');
		    playing();
		});
		$(document.body).on('click', '.hp_pause', function() {
			$(this).toggleClass('hp_pause hp_play').html('<i class="glyphicon glyphicon-play"></i>');
		    clearInterval(foo);
		});
