$(function() {
	...
	var currDir = 'in',
		...
		singleComm, bLabel, commItem, circle_scaler, offset,
		projection = d3.geo.albersUsa().scale(900).translate([dt_width, 430 / 2]);
	var tabs = {'region':'Overview','network':'Network','freight':'Freight Centers','domestic':'Domestic Trade Patterns'},
	...
	d3.json('data/d3/us_states_shapes.json', function(json) {
		...
		function navigation_dots(el, parel){
			var el_str = el.split('-');
				cname = el_str[1];
			...
			parel.closest('li').toggleClass('c-step-current');
			parel.find('svg.c-step-nav circle:first').attr('r', 8);
			parel.find('svg.c-step-nav circle:nth-child(2)').attr('r',3);
			$('.c-region-tab').hide();
			$('#'+el).show();
			$('#c-tab').html(tabs[cname]);
			dt_width = $('#content').width();
			if(el === 'c-domestic-trade'){
				rebuildpage();
		...
		//***************************
		// region interpage navigation handler
		...
		// ********************************
		// initialize county map function
